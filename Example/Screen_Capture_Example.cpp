...
int main()
{
    ...
    std::cout << "Running standard capturing ";
    auto framgrabber =
        ...
        if (img) {
            // std::cout << "New mouse coordinates AND NEW Image received." << " x= " << x << " y= " << y  << std::endl;
            //tje_encode_to_file(s.c_str(), Width(*img), Height(*img), 4, (const unsigned char*)StartSrc(*img));
        }
        else {
            // std::cout << "New mouse coordinates received." << " x= " << x << " y= " << y << " The mouse image is still the same as the last" << std::endl;
        }
